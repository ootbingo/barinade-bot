...
class SrlSyncJob(
  ...
  fun execute() {
    ...
    try {
      ...
      if (e is ResourceAccessException && e.message?.contains("PKIX path validation failed") == true) {
        logger.error("SRL HTTPS certificate expired. {}", e.rootCause?.message)
      } else {
        logger.error("SRL Sync failed.", e)
      }
    }
    ...
    val allResultsOfIncompleteRaces = incompleteDbRaces
      ...
      .flatMap {
        it.second.results.map { result ->
          RaceResult(
            RaceResult.ResultId(it.first, getPlayerWithUsername(result.player)),
            result.place,
            result.time,
